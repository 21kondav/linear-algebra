...
public class AugmentedArray {
	...
	public AugmentedArray(AugmentedArray other) {
		this(other.constants.length, other.variables.length);
		for (int k = 0; k < other.coefficients.length; k++) {
			for (int l = 0; l < other.coefficients[k].length; l++) {
				coefficients[k][l] = other.coefficients[k][l];
			}
		}
		for (int k = 0; k < other.variables.length; k++) {
			variables[k] = other.variables[k];
		}

		for (int k = 0; k < other.constants.length; k++) {
			constants[k] = other.constants[k];
		}
	}
